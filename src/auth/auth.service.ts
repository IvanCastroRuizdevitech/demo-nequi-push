...
export class AuthService {
  ...
  async getToken(): Promise<string | null> {
    try {
        const clientId = await this.parametrosService.obtenerValorParametro(ImplementacionNequi.NEQUI_CLIENT_ID); 
        const clientSecret = await this.parametrosService.obtenerValorParametro(ImplementacionNequi.NEQUI_CLIENT_SECRET); 
        const credentials = Buffer.from(`${clientId}:${clientSecret}`).toString('base64');
        const url = await this.parametrosService.obtenerValorParametro(ImplementacionNequi.NEQUI_AUTH_URI); 
